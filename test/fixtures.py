...
def model(environment, parameters):
    ...
    # interfaces.
    model = delay_out_center_task.Model(environment=environment, parameters=parameters)
...
def trigger(node, client, spin):
    ...
    def trigger(key):
        ...
        return dict(state=state, client_state=client_state, client_event=client_event)
