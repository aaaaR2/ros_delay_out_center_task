...
class Environment(Client):
    ...
    def __init__(
        self,
        *args,  # Timer=threading.Timer,
        Timer=None,
        **kwargs,
    ):
        # self.timer = Timer
        super().__init__(*args, **kwargs)
        ros_timer = self.node.create_timer(timer_period_sec=1, callback=lambda: None)
        ros_timer.cancel()
        Timer = (
            Timer
            if Timer
            else ros_threading_timer.TimerWrapper(ros_timer, node=self.node)
        )
        self.timer = Timer
        self.objects = self
    ...
    def get_radius(self, key):
        return self[key].radius

    def set_radius(self, value, key="cursor"):
        self[key].radius = value
